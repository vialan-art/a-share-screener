...
## 2026-07-06 修复误导性历史回测
- GitHub 已同步两次提交：数据完整性提升 + 滚动回测修复。
- 问题：Overview 页“策略历史表现（等权重 Top 20）”用最新快照的选股名单去回测 1/2/3 年前买入持有至今的收益，导致策略收益 -4.25%、沪深300 +26.01%，超额 -30.26%。
- 原因：这不是真实策略回测，而是用“今天的选股”穿越回历史买入，存在前视偏差；且近期沪深300以高股息、大盘股为主，当前价值/质量选股偏中小盘，造成巨大偏离。
- 修复：Overview 改为展示真实的滚动回测（日频调仓），并明确标注当前快照历史较短、仅反映极短期表现。
- 当前真实滚动回测（2026-06-29 至 2026-07-03）：策略 +3.47%，沪深300 -1.72%，跑赢了指数。
- 后续：积累更多交易日快照后，滚动回测会自动扩展周期数；长期历史表现需要持续运行每日选股任务。